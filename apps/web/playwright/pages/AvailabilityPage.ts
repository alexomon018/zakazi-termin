...
export class AvailabilityPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    this.pageTitle = page.locator('[data-testid="availability-title"], h1:has-text("Dostupnost")').first();
    this.addScheduleButton = page.locator('[data-testid="add-schedule-button"], button:has-text("Novi raspored"), button:has-text("Dodaj"), button:has-text("+")').first();
    this.schedulesList = page.locator('[data-testid="schedules-list"]').first();
    this.timezoneDisplay = page.locator('[data-testid="timezone-display"], text=/[Vv]remenska zona|[Tt]imezone|Europe\\/Belgrade/').first();
    this.saveButton = page.locator('[data-testid="availability-save-button"], button:has-text("Sačuvaj"), button:has-text("Save")').first();
  }
  ...
  getScheduleCard(scheduleName: string): Locator {
    return this.page.getByTestId(`schedule-card-${scheduleName.toLowerCase().replace(/\s+/g, "-")}`);
  }
  ...
  async createSchedule(name: string): Promise<void> {
    ...
    const nameInput = this.page.locator('[data-testid="schedule-name-input"], input[name="name"], input[id="name"]').first();
    await expect(nameInput).toBeVisible();
    await this.fillField(nameInput, name);

    const createButton = this.page.locator('[data-testid="create-schedule-button"], button[type="submit"]').first();
  ...
  getStartTimeInput(dayIndex: number): Locator {
    return this.page.locator(`[data-testid="day-${dayIndex}-start-time"], input[type="time"]`).first();
  }
  ...
  async expectTimezoneDisplayed(): Promise<void> {
    ...
    const hasTimezone =
      ...
      await this.page.locator("text=/Europe|America|Asia/").isVisible().catch(() => false);
  ...
  async expectDaysVisible(): Promise<void> {
    ...
    const hasDays =
      await this.page.locator("text=/[Pp]onedeljak|Monday/").isVisible().catch(() => false) ||
      await this.page.locator("text=/[Uu]torak|Tuesday/").isVisible().catch(() => false) ||
      await this.page.locator("text=/[Ss]reda|Wednesday/").isVisible().catch(() => false);
